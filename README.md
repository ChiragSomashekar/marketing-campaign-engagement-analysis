...
- Constraints: no session identifiers → sessions were **modeled** usign a **30-minute inactivity threshold**

## Method 
- **Sessionization:** for each user, a new session starts if the time since the previous event exceeds 30 minutes.
- **Session duration:** calculated as the difference detween the first and last event timestamp within a session.
...
- **Reliability filtering:** campaign-level analysis excluded with vaey small session counts to reduce noise.
